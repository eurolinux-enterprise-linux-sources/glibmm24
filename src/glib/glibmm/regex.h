 ...
 *  A dollar metacharacter ("$") in the pattern matches only at the end of the string. Without this option, a dollar also matches immediately before the final character if it is a newline (but not before any other newlines). This option is ignored if REGEX_MULTILINE is set.
 * 
 ...
 *  Limits an unanchored pattern to match before (or at) the first newline. @newin{2,34}
 * 
 ...
 *  Changes behaviour so that it is compatible with JavaScript rather than PCRE. @newin{2,34}
 * 
 ...
 *  Turns on the partial matching feature. In contrast to to REGEX_MATCH_PARTIAL_SOFT, this stops matching as soon as a partial match is found, without continuing to search for a possible complete match. See g_match_info_is_partial_match() for more information. @newin{2,34}
 * 
 *  @var RegexMatchFlags REGEX_MATCH_NOTEMPTY_ATSTART
 *  Like REGEX_MATCH_NOTEMPTY, but only applied to the start of the matched string. For anchored patterns this can only happen for pattern containing "\\K". @newin{2,34}
 * 
