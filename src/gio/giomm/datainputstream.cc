...
{
  GError* gerror = nullptr;
  char* tmp = g_data_input_stream_read_line_utf8(gobj(), nullptr, const_cast<GCancellable*>(Glib::unwrap(cancellable)), &(gerror));
  line = tmp;
  g_free(tmp);
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
...
{
  GError* gerror = nullptr;
  char* tmp = g_data_input_stream_read_line_utf8(gobj(), &(length), nullptr, &(gerror));
  line = tmp;
  g_free(tmp);
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
...
{
  GError* gerror = nullptr;
  char* tmp = g_data_input_stream_read_line_utf8(gobj(), nullptr, nullptr, &(gerror));
  line = tmp;
  g_free(tmp);
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
...
{
  GError* gerror = nullptr;
  char* tmp = g_data_input_stream_read_line_finish_utf8(gobj(), Glib::unwrap(result), nullptr, &(gerror));
  data = tmp;
  g_free(tmp);
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
